#!/usr/bin/env python3
"""
Extract YouTube video transcript using rotating proxies
"""
import sys
import json
import subprocess
...
def download_with_proxy(video_id, output_dir, proxy=None):
    """Download transcript using yt-dlp with proxy"""
...
def process_transcript(video_id, transcript_dir):
    ...
    if not transcript_file:
        print(f"Transcript file not found. Tried:")
        for pattern in patterns:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 extract-video-proxy.py <youtube_url> [proxy]")
        sys.exit(1)
    ...
    # Download transcript (with or without proxy)
    if not download_with_proxy(video_id, data_dir, proxy):
        print("Failed to download transcript")
        print("\nTry using a proxy:")
        print(f"  python3 scripts/extract-video-proxy.py '{url}' 'http://proxy:port'")
        sys.exit(1)
